#!/usr/bin/env python3
"""
MiNES v2 — Milestoned Nonequilibrium Switching, Version 2.

Implements the current MiNES protocol with the following corrections over v1:

  Fix 1: BAR-offset-corrected pairwise overlap (not raw sigmoid).
  Fix 2: Protocol path reading supports both 'x0' and 'center_x' columns.
  Fix 3: Robust MTS bootstrap key selection (pmf_ref0/var_ref0 and fallback).
  Fix 4: Global variance = inverse-precision, not arithmetic mean patch variance.
  Fix 5: Adaptive connectivity refinement loop over disconnected EQ cluster pairs.
  Fix 6: Final perturbation work actually appended to trajectory rows for CFT/MTS.
...
class GlobalGTWidthProfile:
    ...
    Computed once from L0 and R0. Not updated per generation.
    """
    m_L0: float
    sigma_L0: float
    m_R0: float
...
# ---------------------------------------------------------------------------
# Local harmonic background fit (mean-only)
# ---------------------------------------------------------------------------

def fit_local_harmonic_mean_only(
    left_window: EnsembleWindow, right_window: EnsembleWindow
) -> dict[str, Any]:
    """Estimate background harmonic k0, x0 from mean-only force balance."""
...
def design_child_window(
    *,
    ...
    No force matching. Priority: preserve m_next > keep k in bounds > match sigma.
    """
...
def design_midpoint_refinement_window(
    left_boundary: EnsembleWindow,
    right_boundary: EnsembleWindow,
    profile: GlobalGTWidthProfile,
    k_min: float,
    ...
    """Design a midpoint EQ refinement window for a disconnected pair.

    Uses target mean = midpoint between boundary window means.
    Same bias construction priority as child design: preserve m_target.
    """
    ...
    if abs(k_child) > 1e-12:
        x_child = ((k0 + k_child) * m_target - k0 * x0) / k_child
    else:
        x_child = m_target
    ...
    return {
        ...
        "bg_fit_valid": int(bg["fit_valid"]),
    }


# ---------------------------------------------------------------------------
# Fix 2: Robust protocol path reading (supports 'x0' and 'center_x')
# ---------------------------------------------------------------------------

def read_protocol_centers_and_k(path_file: Path) -> tuple[np.ndarray, np.ndarray]:
    """Read protocol path file. Supports both 'x0' (adaptive_methods.write_protocol_path)
    and 'center_x' column names."""
    ...
    if x_key is None:
        raise RuntimeError(
            f"Protocol path {path_file} has no center column. "
...
# ---------------------------------------------------------------------------
# Fix 3: Robust MTS bootstrap key selection
# ---------------------------------------------------------------------------

def get_bootstrap_pmf_and_variance(
    boot_result: dict[str, Any]
) -> tuple[np.ndarray, np.ndarray]:
    """Extract PMF and variance from bootstrap_bidirectional_mts_pmf result.

    Supports both the current key names (pmf_ref0/var_ref0) and legacy names
    (pmf/boot_var) for compatibility.
    """
    if "pmf_ref0" in boot_result and "var_ref0" in boot_result:
        return (
            np.asarray(boot_result["pmf_ref0"], dtype=float),
            np.asarray(boot_result["var_ref0"], dtype=float),
        )
    if "pmf" in boot_result and "boot_var" in boot_result:
        return (
            np.asarray(boot_result["pmf"], dtype=float),
            np.asarray(boot_result["boot_var"], dtype=float),
        )
    raise RuntimeError(
        f"Unknown bootstrap_bidirectional_mts_pmf return keys: {sorted(boot_result.keys())}"
    )


# ---------------------------------------------------------------------------
# Fix 1: BAR-offset-corrected pairwise EQ overlap
# ---------------------------------------------------------------------------

def compute_bar_mbar_overlap_v2(
    ...
    Uses BAR to estimate the free-energy offset delta_f, then applies the
    offset-corrected acceptance formula:

        O_ij = mean_{x~L} 1 / (1 + exp( u_R(x) - u_L(x) - delta_f ))
        O_ji = mean_{x~R} 1 / (1 + exp( u_L(x) - u_R(x) + delta_f ))
        O_pair = min(O_ij, O_ji)

    where delta_f = F_R - F_L from BAR. This is the correct two-state MBAR
    acceptance fraction that accounts for the free-energy difference between
    the biased ensembles.

    Falls back to raw sigmoid (without offset) when BAR fails, and marks
    overlap_method = 'raw_sigmoid_no_bar'.
    """
    _FAIL: dict[str, Any] = {
    ...
    try:
        ...
        # Instantaneous perturbation works (single step): shape (n,) → (n, 1) for BAR solver
        w_LR = (u_R(x_L) - u_L(x_L)).reshape(-1, 1)  # forward: L → R on L-samples
        w_RL = (u_L(x_R) - u_R(x_R)).reshape(-1, 1)  # reverse: R → L on R-samples

        cft = solve_segment_cft_delta_f_once(w_LR, w_RL, kT=kT)
        bar_solved = bool(cft.get("cft_solved", False))

        if bar_solved and cft.get("delta_f") is not None and math.isfinite(float(cft["delta_f"])):
            delta_f = float(cft["delta_f"])
            # Offset-corrected acceptance fractions
            ...
            # Fallback: raw sigmoid without BAR offset
            delta_f = float("nan")
            arg_L = np.clip(u_R(x_L) - u_L(x_L), -500, 500)
            arg_R = np.clip(u_L(x_R) - u_R(x_R), -500, 500)
            O_ij = float(np.mean(1.0 / (1.0 + np.exp(arg_L))))
            O_ji = float(np.mean(1.0 / (1.0 + np.exp(arg_R))))
            overlap_method = "raw_sigmoid_no_bar"
        ...
        return {
            ...
            "bar_delta_f": float(cft.get("delta_f", float("nan"))) if bar_solved else "",
            "bar_delta_f_unc": float(cft.get("delta_f_unc", float("nan"))) if bar_solved else "",
...
# ---------------------------------------------------------------------------
# EQ clustering: BAR/MBAR overlap-based (JSD as diagnostic only)
# ---------------------------------------------------------------------------

def build_eq_clusters_v2(
    windows: list[EnsembleWindow],
    ctx: dict[str, Any],
    overlap_threshold: float,
) -> tuple[list[EQCluster], list[dict[str, Any]]]:
    """Merge neighboring EQ windows into clusters by BAR-offset-corrected MBAR overlap.

    Connectivity: O_pair >= overlap_threshold (default 0.3).
    BAR must be solved; if BAR fails, raw sigmoid is used with a warning.
    JSD is written as a diagnostic column and does not drive connectivity.
    """
    ...
    for window in ordered[1:]:
        ...
        overlap = compute_bar_mbar_overlap_v2(left_window, window, ctx)
...
# ---------------------------------------------------------------------------
# NEQ bridge protocol: linear interpolation
# ---------------------------------------------------------------------------

def build_linear_bridge_protocol_v2(
    left_window: EnsembleWindow,
    right_window: EnsembleWindow,
    n_time: int,
    k_min: float,
...
# ---------------------------------------------------------------------------
# Fix 6: Final perturbation work appending
# ---------------------------------------------------------------------------

def append_final_perturbation_to_trajectory_rows(
    trajectories: list[list[dict[str, str]]],
    *,
    endpoint_center: float,
    endpoint_k: float,
    last_protocol_center: float,
    last_protocol_k: float,
    direction_label: str,
) -> tuple[list[list[dict[str, str]]], list[dict[str, Any]]]:
    """Append a final instantaneous perturbation work step to each trajectory.

    For each trajectory ending at coordinate x_T with cumulative work W_old,
    compute the extra work of switching from the last protocol bias to the
    endpoint bias:

        dW = 0.5 * k_endpoint * (x_T - cx_endpoint)^2
           - 0.5 * k_last    * (x_T - cx_last)^2

    Append a synthetic row with W_new = W_old + dW.

    Returns (augmented trajectories, per-trajectory summary rows).
    """
    ...
    for traj_idx, rows in enumerate(trajectories):
        ...
        # Build the synthetic final perturbation row
        try:
            old_step = int(last_row.get("step", 0))
        except (ValueError, TypeError):
            old_step = len(rows)
        ...
        summary_rows.append({
            "direction": direction_label, "traj_index": traj_idx,
            "x_final": float(x_T), "old_work": float(W_old), "dW_final": float(dW),
            "new_work": float(W_new),
            "endpoint_center": float(endpoint_center), "endpoint_k": float(endpoint_k),
            "last_protocol_center": float(last_protocol_center),
            "last_protocol_k": float(last_protocol_k),
            "appended": 1,
        })
...
# ---------------------------------------------------------------------------
# NEQ segment runner (uses Fix 6)
# ---------------------------------------------------------------------------

def run_neq_segment_v2(
    *,
    name: str,
    left_boundary: EnsembleWindow,
    right_boundary: EnsembleWindow,
    ...
    Appends a final instantaneous perturbation work step to each trajectory so
    that the total work in the work array includes the perturbation from the
    last protocol step to the actual endpoint bias (Fix 6).
    """
    root.mkdir(parents=True, exist_ok=True)
    n_time = max(2, int(t_neq))
    protocol = build_linear_bridge_protocol_v2(
    ...
    # Fix 6: append final perturbation to each trajectory
    fwd_trajs, fwd_perturb_rows = append_final_perturbation_to_trajectory_rows(
        fwd_trajs_raw,
        endpoint_center=float(right_boundary.center_x),
        endpoint_k=float(right_boundary.k),
        last_protocol_center=float(centers_fwd[-1]),
        last_protocol_k=float(ks_fwd[-1]),
        direction_label="forward",
    )
    rev_trajs, rev_perturb_rows = append_final_perturbation_to_trajectory_rows(
        rev_trajs_raw,
        endpoint_center=float(left_boundary.center_x),
        endpoint_k=float(left_boundary.k),
        last_protocol_center=float(centers_rev[-1]),
        last_protocol_k=float(ks_rev[-1]),
        direction_label="reverse",
    )

    # Write final perturbation diagnostics
    perturb_rows = fwd_perturb_rows + rev_perturb_rows
    if perturb_rows:
        _perturb_cols = [
        ...
        perturb_path = root / "final_perturbation_summary.csv"
        write_csv(perturb_path, _perturb_cols, perturb_rows)

    n_fwd_appended = sum(1 for r in fwd_perturb_rows if r.get("appended", 0))
    n_rev_appended = sum(1 for r in rev_perturb_rows if r.get("appended", 0))
    ...
    write_json(root / "segment_summary.json", {
        ...
        "n_final_perturbations_forward": n_fwd_appended,
        "n_final_perturbations_reverse": n_rev_appended,
        "n_forward_trajs": len(fwd_trajs),
        "n_reverse_trajs": len(rev_trajs),
    })
    return segment
...
def build_eq_cluster_patch_v2(
    ...
    write_csv(patch_dir / "pmf.csv", ["x", "pmf", "variance"], pmf_rows)
    ...
    return PMFPatch(
        ...
        metadata={
            "cluster_name": cluster.name, "window_names": [w.name for w in cluster.windows],
            "left_x": float(cluster.left_x), "right_x": float(cluster.right_x), "n_boot": int(n_boot),
        },
...
# ---------------------------------------------------------------------------
# NEQ/MTS patch (Fix 3: robust key selection)
# ---------------------------------------------------------------------------

def build_neq_mts_patch_v2(
    segment: NEQSegment,
    grid: np.ndarray,
    ctx: dict[str, Any],
    n_boot: int,
    ...
    """Build NEQ/MTS patch. No NES truncation. Uses pmf_ref0/var_ref0 keys (Fix 3)."""
    ...
    fixed_delta_f = (
        float(cft["delta_f"])
        if cft_solved and cft.get("delta_f") is not None and math.isfinite(float(cft["delta_f"]))
        else None
    )
    ...
    segment.cft_summary = {
        ...
        "delta_f_unc": cft.get("delta_f_unc"), "method": cft.get("method"),
        "reason": cft.get("reason"), "mts_solved": 1,
        "neq_pair_source": segment.connectivity.get("neq_pair_source", ""),
        "final_perturbation_appended": segment.connectivity.get("final_perturbation_appended", True),
        "n_boot_used": boot_result.get("n_boot_used", 0),
        "bootstrap_recomputed_cft": boot_result.get("bootstrap_recomputed_cft", 0),
    }
    ...
    write_json(patch_dir / "patch_summary.json", {
        ...
        "cft_solved": cft_solved, "delta_f": cft.get("delta_f"), "n_boot": int(n_boot),
        "neq_pair_source": segment.connectivity.get("neq_pair_source", ""),
        "final_perturbation_appended": True,
    })
    ...
    return PMFPatch(
        ...
        metadata={
            ...
            "neq_pair_source": segment.connectivity.get("neq_pair_source", ""),
            "mts_solved": 1, "final_perturbation_appended": True,
        },
    )


# ---------------------------------------------------------------------------
# Fix 4: Global PMF fusion with inverse-precision variance
# ---------------------------------------------------------------------------

def fit_global_pmf_v2(
    patches: list[PMFPatch], grid: np.ndarray, variance_floor: float,
) -> tuple[np.ndarray, np.ndarray, dict[str, Any]]:
    """Inverse-variance-weighted global PMF fit.

    Solves: min_{G,c} sum_p sum_x (G(x) - F_p(x) - c_p)^2 / (var_p(x) + eps)

    Global variance = 1 / sum_p [1 / (var_p(x) + eps)]  (inverse-precision, not mean).
    """
    ...
    # Fix 4: inverse-precision variance
    precision_sum = np.zeros(n_grid, dtype=float)
    n_cover = np.zeros(n_grid, dtype=int)
    for pi, gi, _, _ in obs:
        v = float(patches[pi].variance[gi])
        precision_sum[gi] += 1.0 / (v + float(variance_floor))
        n_cover[gi] += 1
...
def write_state_tables_v2(
    ...
    write_csv(base_root / "global_pmf.csv", _PMF_COLS, pmf_rows)
...
# ---------------------------------------------------------------------------
# Fix 5: Adaptive connectivity refinement loop
# ---------------------------------------------------------------------------

_REFINEMENT_COLS = [
    "round", "target_pair", "left_window", "right_window",
    "left_mean_x", "right_mean_x", "segment_type", "nes_action",
    "mts_solved", "inserted_window", "inserted_center_x", "inserted_k",
...
def _find_existing_segment(
    segments: list[NEQSegment],
    left_boundary_name: str,
    right_boundary_name: str,
) -> NEQSegment | None:
    """Return an existing segment that covers the given boundary pair, or None."""
    for seg in segments:
        if (seg.left_boundary.name == left_boundary_name
                and seg.right_boundary.name == right_boundary_name):
            return seg
    return None


def run_connectivity_refinement(
    *,
    windows: list[EnsembleWindow],
    clusters: list[EQCluster],
    segments: list[NEQSegment],
    ...
    """Adaptive connectivity refinement over disconnected EQ cluster pairs.

    For each disconnected neighboring cluster pair:
    1. Check if existing bidirectional NES covers the boundary pair.
    2. If yes, reuse it (already has final perturbation appended).
       If no, run new bidirectional NES with final perturbation appended.
    3. Build MTS patch for the segment.
    4. Insert a midpoint EQ refinement window if budget allows.
    5. Run EQ for the inserted window.
    6. Rebuild clusters.
    Repeat until connected or budget/rounds exhausted.

    neq_pair_source is recorded as 'existing_bidirectional' or 'newly_generated'.
    No NES truncation.
    """
    ...
    for round_idx in range(max_rounds):
        ...
        # Sort clusters by their leftmost mean_x
        sorted_clusters = sorted(clusters, key=lambda c: float(c.left_x))

        made_progress = False
        for ci in range(len(sorted_clusters) - 1):
            left_cluster = sorted_clusters[ci]
            right_cluster = sorted_clusters[ci + 1]

            # Boundary windows: rightmost of left cluster, leftmost of right cluster
            ...
            row: dict[str, Any] = {
                ...
                "left_mean_x": float(left_boundary.mean_x),
                "right_mean_x": float(right_boundary.mean_x),
                "segment_type": "unknown", "nes_action": "",
                "mts_solved": 0, "inserted_window": "",
                "inserted_center_x": "", "inserted_k": "",
                "target_mean": "", "target_sigma": "",
                "k_raw": "", "k_clipped": "",
                "reason": "", "n_clusters_before": n_clusters_before,
                "n_clusters_after": n_clusters_before,
                "used_steps": int(budget.used_steps),
            }

            # Check if existing segment covers this boundary pair
            existing_seg = _find_existing_segment(
                segments, left_boundary.name, right_boundary.name
            )
            ...
            if existing_seg is not None:
                ...
                seg_name = f"refine_r{round_idx}_{pair_label}"
                ...
                seg = run_neq_segment_v2(
                    name=seg_name,
                    left_boundary=left_boundary, right_boundary=right_boundary,
                    left_source=left_cluster, right_source=right_cluster,
                    bin_path=bin_path, ctx=ctx, t_neq=args.t_neq,
                    n_neq_traj=args.n_neq_traj,
                    seed=int(args.seed) + 500000 + round_idx * 100 + ci,
            ...
            # Build MTS patch (diagnostics + for provisional PMF)
            patch_root = out_root / "refinement" / f"round_{round_idx:03d}" / "patches"
            mts_patch = build_neq_mts_patch_v2(
                seg, grid, ctx, int(args.n_bootstrap_neq), patch_root,
                rng_seed=int(args.seed) + 600000 + round_idx * 100 + ci,
            )
            row["mts_solved"] = int(bool(seg.cft_summary.get("cft_solved", False)))

            # Classify segment type using local harmonic fit
            bg = fit_local_harmonic_mean_only(left_boundary, right_boundary)
            k0 = float(bg["k0"]) if bg["fit_valid"] and math.isfinite(bg["k0"]) else float("nan")
            x0 = float(bg["x0"]) if bg["fit_valid"] and math.isfinite(bg["x0"]) else float("nan")
            is_transition = (
            ...
            # Insert midpoint EQ refinement window
            design = design_midpoint_refinement_window(
                left_boundary, right_boundary, profile,
                float(args.k_min), float(args.k_max), beta_eff,
            )
            row["target_mean"] = float(design["target_mean"])
            row["target_sigma"] = float(design["target_sigma"])
            row["k_raw"] = float(design["k_raw"])
            row["k_clipped"] = int(design["k_clipped"])
            ...
            win_name = f"refine_r{round_idx}_p{ci}_win"
            ...
            new_window = run_eq_window_v2(
                name=win_name, center_x=float(design["x_child"]),
                k=float(design["k_child"]), generation=-1, side="refine",
                bin_path=bin_path, ctx=ctx, n_eq_steps=args.n_eq_steps,
                eq_save_every=args.eq_save_every, tail_fraction=args.tail_fraction,
                seed=int(args.seed) + 700000 + round_idx * 100 + ci,
                root=win_root,
            )
            windows.append(new_window)
            row["inserted_window"] = win_name
            row["inserted_center_x"] = float(design["x_child"])
            row["inserted_k"] = float(design["k_child"])

            # Rebuild clusters after inserting the window
            clusters, overlap_rows = build_eq_clusters_v2(windows, ctx, float(args.eq_overlap_threshold))
            row["n_clusters_after"] = len(clusters)
            row["reason"] = "window_inserted"

            # Rebuild EQ patches and provisional PMF
            eq_patches: list[PMFPatch] = []
            for ci2, cl in enumerate(clusters):
                eq_patches.append(build_eq_cluster_patch_v2(
                    cl, grid, ctx, int(args.n_bootstrap_eq),
                    out_root / "refinement" / f"round_{round_idx:03d}" / "patches" / "eq",
                    rng_seed=int(args.seed) + 400000 + round_idx * 1000 + ci2,
                ))
            ...
            global_pmf, global_var, fit_details = fit_global_pmf_v2(
            ...
            quality_rows.append(compute_pmf_quality_v2(
                grid, global_pmf, global_var, ctx, int(budget.used_steps),
                f"refinement_round_{round_idx}_pair_{ci}",
            ))
    ...
    *,
    ...
    while True:
        ...
        for wi, window in enumerate(selected_windows):
            ...
            tail_x = np.asarray([float(r["x"]) for r in window.tail_rows if r.get("x", "") != ""],
                                 dtype=float)
        ...
        budget.spend(round_cost, f"final_eq_extension_round_{round_index:03d}", "EQ_EXTENSION",
                     "final_eq_extension")

        new_clusters, new_overlap = build_eq_clusters_v2(windows, ctx, float(args.eq_overlap_threshold))
        ...
        if not eq_network_is_connected(new_clusters):
            ...
            write_json(out_root / "eq_connectivity_lost.json", {
                "round": round_index, "n_clusters_after_extension": len(new_clusters),
                "cluster_names": [c.name for c in new_clusters], "n_windows": len(windows),
            })
            return clusters, patches_for_global, global_pmf, global_var, fit_details, overlap_rows, ext_rows
        ...
        for ci, cl in enumerate(clusters):
            new_patches.append(build_eq_cluster_patch_v2(
        ...
        write_state_tables_v2(round_root / "state", windows, clusters, segments,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="MiNES v2 — BAR-offset overlap, final perturbation appending, "
                    "adaptive refinement loop, inverse-precision variance."
    )
    parser.add_argument("--system-root", required=True)
    parser.add_argument("--bin", required=True)
    parser.add_argument("--seed", required=True, type=int)
    parser.add_argument("--label", default="mines_variance_fusion_v2")
    ...
    parser.add_argument("--target-kl", default=1.0, type=float,
                        help="Target KL distance for basin-like KL-GT exploration steps.")
    parser.add_argument("--eq-overlap-threshold", default=0.3, type=float,
                        help="BAR/MBAR pairwise overlap cutoff for EQ window connectivity.")
    parser.add_argument("--max-generations", default=10, type=int)
    parser.add_argument("--max-refinement-rounds", default=10, type=int,
                        help="Maximum rounds in adaptive connectivity refinement loop.")
    ...
    parser.add_argument("--pmf-method", choices=["neq", "eq", "hybrid"], default="neq",
                        help="Provisional PMF estimator (disconnected state only; "
                             "overridden to connected_EQ_MBAR_only when connected).")
    parser.add_argument("--final-refinement-mode", choices=["none", "eq-extend"],
                        default="eq-extend")
...
def main() -> None:
    ...
    # Build globally fixed GT width profile from L0 and R0 (computed once, never updated)
    ...
    # -----------------------------------------------------------------------
    # Stage 1: Exploratory chain growth (seeding, not responsible for full connectivity)
    # -----------------------------------------------------------------------
    clusters, overlap_rows = build_eq_clusters_v2(windows, ctx, float(args.eq_overlap_threshold))
    patches_for_global: list[PMFPatch] = []
    ...
    for generation in range(int(args.max_generations)):
        clusters, overlap_rows = build_eq_clusters_v2(windows, ctx, float(args.eq_overlap_threshold))
        if eq_network_is_connected(clusters):
            print(f"[v2] EQ network connected at generation {generation}. Stopping exploration.")
            break
        ...
        if not budget.can_spend(n_cost):
            print("[v2] Budget exhausted before NEQ at generation", generation)
            break
        ...
        if not budget.can_spend(gen_eq_cost):
            print("[v2] Budget exhausted before EQ children at generation", generation)
            break
        ...
        write_state_tables_v2(gen_root, windows, clusters, segments,
                               patches_for_global, fit_details, overlap_rows,
                               grid, global_pmf, global_var)
        quality_rows.append(compute_pmf_quality_v2(
    ...
    for ci, cl in enumerate(clusters):
        eq_patches.append(build_eq_cluster_patch_v2(
            cl, grid, ctx, int(args.n_bootstrap_eq),
            out_root / "patches" / "pre_refinement" / "eq",
            rng_seed=seed + 450000 + ci,
        ))
    if eq_network_is_connected(clusters):
        patches_for_global = eq_patches
        patch_selection_rule = "connected_EQ_MBAR_only"
    elif args.pmf_method == "neq":
        neq_patches_all = [
            build_neq_mts_patch_v2(s, grid, ctx, int(args.n_bootstrap_neq),
                                    out_root / "patches" / "pre_refinement" / "neq",
                                    rng_seed=seed + 460000 + si)
            for si, s in enumerate(segments) if s.forward_trajectories
        ]
        patches_for_global = neq_patches_all if neq_patches_all else eq_patches
        patch_selection_rule = "only_NEQ_MTS" if neq_patches_all else "EQ_MBAR_only_provisional"
    elif args.pmf_method == "hybrid":
        neq_patches_all = [
            build_neq_mts_patch_v2(s, grid, ctx, int(args.n_bootstrap_neq),
                                    out_root / "patches" / "pre_refinement" / "neq",
    ...
    (windows, clusters, segments, patches_for_global,
     global_pmf, global_var, fit_details, overlap_rows, ref_rows) = run_connectivity_refinement(
    ...
    summary = {
        ...
        "protocol": "mines_variance_fusion_v2",
